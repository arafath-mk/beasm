...
class QuickBooksMigrator(Document):
	...
	def _migrate(self):
		try:
			...
			# QuickBooks data can have transactions that do not fall in existing fiscal years in BEASM
	...
	def _save_account(self, account):
		...
		# Map Quickbooks Account Types to BEASM root_accunts and and root_type
		try:
			if not frappe.db.exists(
