...
frappe.ui.form.on("Delivery Note", {
	setup: function(frm) {
		...
		beasm.queries.setup_queries(frm, "Warehouse", function() {
			return beasm.queries.warehouse(frm.doc);
		});
		beasm.queries.setup_warehouse_query(frm);

		frm.set_query('project', function(doc) {
			return {
				query: "beasm.controllers.queries.get_project_name",
				filters: {
		...
		frm.set_query('expense_account', 'items', function(doc, cdt, cdn) {
			if (beasm.is_perpetual_inventory_enabled(doc.company)) {
				return {
					filters: {
	...
	print_without_amount: function(frm) {
		beasm.stock.delivery_note.set_print_hide(frm.doc);
	},
		...
		if (frm.doc.docstatus == 1 && !frm.doc.inter_company_reference) {
			...
			if (internal) {
				...
				frm.add_custom_button(__(button_label), function() {
					frappe.model.open_mapped_doc({
						method: 'beasm.stock.doctype.delivery_note.delivery_note.make_inter_company_purchase_receipt',
						frm: frm,
					});
				}, __('Create'));
			}
...
beasm.stock.DeliveryNoteController = class DeliveryNoteController extends beasm.selling.SellingController {
	setup(doc) {
	...
	refresh(doc, dt, dn) {
				...
				this.frm.add_custom_button(__('Sales Order'),
					function() {
						...
						beasm.utils.map_current_doc({
							method: "beasm.selling.doctype.sales_order.sales_order.make_delivery_note",
			...
			if(doc.docstatus==0 && !doc.__islocal) {
				if (doc.__onload && doc.__onload.has_unpacked_items) {
					this.frm.add_custom_button(__('Packing Slip'), function() {
						frappe.model.open_mapped_doc({
							method: "beasm.stock.doctype.delivery_note.delivery_note.make_packing_slip",
							frm: me.frm
						}) }, __('Create')
					);
				}
		...
		beasm.stock.delivery_note.set_print_hide(doc, dt, dn);
	}

	make_shipment() {
		frappe.model.open_mapped_doc({
			method: "beasm.stock.doctype.delivery_note.delivery_note.make_shipment",
			frm: this.frm
		})
	}
	...
	make_installation_note() {
		frappe.model.open_mapped_doc({
			method: "beasm.stock.doctype.delivery_note.delivery_note.make_installation_note",
			frm: this.frm
		});
	}
	...
	make_sales_return() {
		frappe.model.open_mapped_doc({
			method: "beasm.stock.doctype.delivery_note.delivery_note.make_sales_return",
			frm: this.frm
		})
	}
	...
	make_delivery_trip() {
		frappe.model.open_mapped_doc({
			method: "beasm.stock.doctype.delivery_note.delivery_note.make_delivery_trip",
			frm: cur_frm
		})
	}
	...
	update_status(status) {
		...
		frappe.call({
			method:"beasm.stock.doctype.delivery_note.delivery_note.update_delivery_note_status",
...
extend_cscript(cur_frm.cscript, new beasm.stock.DeliveryNoteController({frm: cur_frm}));
...
frappe.ui.form.on('Delivery Note', {
	...
	unhide_account_head: function(frm) {
		...
		var aii_enabled = beasm.is_perpetual_inventory_enabled(frm.doc.company)
		frm.fields_dict["items"].grid.set_column_disp(["expense_account", "cost_center"], aii_enabled);
	}
})


beasm.stock.delivery_note.set_print_hide = function(doc, cdt, cdn){
