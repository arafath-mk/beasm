...
beasm.taxes_and_totals = class TaxesAndTotals extends beasm.payments {
	setup() {
	...
	initialize_taxes() {
		...
		$.each(this.frm.doc["taxes"] || [], function(i, tax) {
			...
			if (!this.discount_amount_applied) {
				beasm.accounts.taxes.validate_taxes_and_charges(tax.doctype, tax.name);
				beasm.accounts.taxes.validate_inclusive_tax(tax);
			}
	...
	fetch_round_off_accounts() {
		...
		if (me.frm.doc.company) {
			return frappe.call({
				"method": "beasm.controllers.taxes_and_totals.get_round_off_applicable_accounts",
				"args": {
