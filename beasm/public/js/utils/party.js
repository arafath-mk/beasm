...
beasm.utils.get_party_details = function(frm, method, args, callback) {
	if (!method) {
		method = "beasm.accounts.party.get_party_details";
	}
	...
	if (frappe.meta.get_docfield(frm.doc.doctype, "taxes")) {
		if (!beasm.utils.validate_mandatory(frm, "Posting / Transaction Date",
			args.posting_date, args.party_type=="Customer" ? "customer": "supplier")) return;
	}

	if (!beasm.utils.validate_mandatory(frm, "Company", frm.doc.company, args.party_type=="Customer" ? "customer": "supplier")) {
		return;
	}
...
beasm.utils.add_item = function(frm) {
	if (frm.is_new()) {
...
beasm.utils.get_address_display = function(frm, address_field, display_field, is_your_company_address) {
...
beasm.utils.set_taxes_from_address = function(frm, triggered_from_field, billing_address_field, shipping_address_field) {
	if (frm.updating_party_details) return;

	if (frappe.meta.get_docfield(frm.doc.doctype, "taxes")) {
		if (!beasm.utils.validate_mandatory(frm, "Lead / Customer / Supplier",
	...
	frappe.call({
		method: "beasm.accounts.party.get_address_tax_category",
		args: {
		...
		callback: function(r) {
			if (!r.exc){
				if (frm.doc.tax_category != r.message) {
					...
					beasm.utils.set_taxes(frm, triggered_from_field);
				}
			}
		}
	});
...
beasm.utils.set_taxes = function(frm, triggered_from_field) {
	if (frappe.meta.get_docfield(frm.doc.doctype, "taxes")) {
		if (!beasm.utils.validate_mandatory(frm, "Company", frm.doc.company, triggered_from_field)) {
			return;
		}
	...
	frappe.call({
		method: "beasm.accounts.party.set_taxes",
		args: {
			"party": party,
			"party_type": party_type,
...
beasm.utils.get_contact_details = function (frm) {
	if (frm.updating_party_details) return;
...
beasm.utils.validate_mandatory = function(frm, label, value, trigger_on) {
	if (!value) {
		frm.doc[trigger_on] = "";
...
beasm.utils.get_shipping_address = function(frm, callback) {
	if (frm.doc.company) {
		...
		frappe.call({
			method: "beasm.accounts.custom.address.get_shipping_address",
			args: {
				company: frm.doc.company,
