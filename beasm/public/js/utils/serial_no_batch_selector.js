beasm.SerialBatchPackageSelector = class SerialNoBatchBundleUpdate {
	...
	download_csv_file() {
		...
		const method = `/api/method/beasm.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle.download_blank_csv_template?content=${encodeURIComponent(JSON.stringify(csvFileData))}`;
	...
	upload_csv_file() {
		...
		frappe.call({
			method: 'beasm.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle.upload_csv_file',
			args: {
				item_code: this.item.item_code,
				file_path: file_path
			},
	...
	get_auto_data() {
		...
		if (qty) {
			frappe.call({
				method: 'beasm.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle.get_auto_data',
				args: {
					item_code: this.item.item_code,
	...
	update_bundle_entries() {
		...
		frappe.call({
			method: 'beasm.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle.add_serial_batch_ledgers',
			args: {
				entries: entries,
				child_row: this.item,
				doc: this.frm.doc,
	...
	render_data() {
		if (!this.frm.is_new() && this.bundle) {
			frappe.call({
				method: 'beasm.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle.get_serial_batch_ledgers',
				args: {
					item_code: this.item.item_code,
					name: this.bundle,
