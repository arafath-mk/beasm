...
frappe.provide("beasm.queries");
$.extend(beasm.queries, {
	user: function() {
	...
	item: function(filters) {
		var args = { query: "beasm.controllers.queries.item_query" };
		if(filters) args["filters"] = filters;
		return args;
	},

	bom: function() {
		return { query: "beasm.controllers.queries.bom" };
	},

	task: function() {
		return { query: "beasm.projects.utils.query_task" };
	},
	...
	employee: function() {
		return { query: "beasm.controllers.queries.employee_query" }
	},
	...
	get_filtered_dimensions: function(doc, child_fields, dimension, company) {
		...
		return {
			query: "beasm.controllers.queries.get_filtered_dimensions",
			filters: {
				'dimension': dimension,
				'account': account,
				'company': company
...
beasm.queries.setup_queries = function(frm, options, query_fn) {
	var me = this;
...
*/
beasm.queries.setup_warehouse_query = function(frm){
	frm.set_query('warehouse', 'items', function(doc, cdt, cdn) {
		var row  = locals[cdt][cdn];
		var filters = beasm.queries.warehouse(frm.doc);
		if(row.item_code){
			$.extend(filters, {"query":"beasm.controllers.queries.warehouse_query"});
