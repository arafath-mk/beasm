...
frappe.provide("beasm.crm");
beasm.pre_sales.set_as_lost("Opportunity");
...
frappe.ui.form.on("Opportunity", {
	...
	party_name: function(frm) {
		...
		if (frm.doc.opportunity_from == "Customer") {
			beasm.utils.get_party_details(frm);
		} else if (frm.doc.opportunity_from == "Lead") {
			beasm.utils.map_current_doc({
				method: "beasm.crm.doctype.lead.lead.make_opportunity",
	...
	customer_address: function(frm, cdt, cdn) {
		beasm.utils.get_address_display(frm, 'customer_address', 'address_display', false);
	},

	contact_person: beasm.utils.get_contact_details,
	...
	currency: function(frm) {
		let company_currency = beasm.get_currency(frm.doc.company);
	...
	make_supplier_quotation: function(frm) {
		frappe.model.open_mapped_doc({
			method: "beasm.crm.doctype.opportunity.opportunity.make_supplier_quotation",
			frm: frm
		})
	},
	...
	make_request_for_quotation: function(frm) {
		frappe.model.open_mapped_doc({
			method: "beasm.crm.doctype.opportunity.opportunity.make_request_for_quotation",
			frm: frm
		})
	},
...
beasm.crm.Opportunity = class Opportunity extends frappe.ui.form.Controller {
	onload() {
	...
	setup_queries() {
		...
		if (me.frm.doc.opportunity_from == "Lead") {
			me.frm.set_query('party_name', beasm.queries['lead']);
		}
		else if (me.frm.doc.opportunity_from == "Customer") {
			me.frm.set_query('party_name', beasm.queries['customer']);
	...
	create_quotation() {
		frappe.model.open_mapped_doc({
			method: "beasm.crm.doctype.opportunity.opportunity.make_quotation",
			frm: cur_frm
		})
	}
	...
	make_customer() {
		frappe.model.open_mapped_doc({
			method: "beasm.crm.doctype.opportunity.opportunity.make_customer",
			frm: cur_frm
		})
	}

	show_notes() {
		const crm_notes = new beasm.utils.CRMNotes({
			frm: this.frm,
	...
	show_activities() {
		const crm_activities = new beasm.utils.CRMActivities({
			frm: this.frm,
...
extend_cscript(cur_frm.cscript, new beasm.crm.Opportunity({frm: cur_frm}));

cur_frm.cscript.item_code = function(doc, cdt, cdn) {
	var d = locals[cdt][cdn];
	if (d.item_code) {
		return frappe.call({
			method: "beasm.crm.doctype.opportunity.opportunity.get_item_details",
