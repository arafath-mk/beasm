...
beasm.accounts.taxes.setup_tax_validations("Sales Order");
...
frappe.ui.form.on("Sales Order", {
		...
		if(frm.doc.docstatus === 1) {
			if (frm.doc.status !== 'Closed' && flt(frm.doc.per_delivered, 2) < 100 && flt(frm.doc.per_billed, 2) < 100) {
				frm.add_custom_button(__('Update Items'), () => {
					beasm.utils.update_child_items({
						frm: frm,
						child_docname: "items",
	...
	get_items_from_internal_purchase_order(frm) {
		frm.add_custom_button(__('Purchase Order'), () => {
			beasm.utils.map_current_doc({
				method: 'beasm.buying.doctype.purchase_order.purchase_order.make_inter_company_sales_order',
	...
	onload: function(frm) {
		...
		frm.set_query('warehouse', 'items', function(doc, cdt, cdn) {
			...
			if (row.item_code) {
				query.query = "beasm.controllers.queries.warehouse_query";
	...
	cancel_stock_reservation_entries(frm) {
		frappe.call({
			method: 'beasm.stock.doctype.stock_reservation_entry.stock_reservation_entry.cancel_stock_reservation_entries',
			args: {
...
frappe.ui.form.on("Sales Order Item", {
	...
	delivery_date: function(frm, cdt, cdn) {
		if(!frm.doc.delivery_date) {
			beasm.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "items", "delivery_date");
		}
	}
});

beasm.selling.SalesOrderController = class SalesOrderController extends beasm.selling.SellingController {
	onload(doc, dt, dn) {
	...
	refresh(doc, dt, dn) {
		...
		if (this.frm.doc.docstatus===0) {
			this.frm.add_custom_button(__('Quotation'),
				function() {
					let d = beasm.utils.map_current_doc({
						method: "beasm.selling.doctype.quotation.quotation.make_sales_order",
	...
	create_pick_list() {
		frappe.model.open_mapped_doc({
			method: "beasm.selling.doctype.sales_order.sales_order.create_pick_list",
			frm: this.frm
		})
	}
	...
	make_work_order() {
		var me = this;
		me.frm.call({
			method: "beasm.selling.doctype.sales_order.sales_order.get_work_order_items",
			args: {
	...
	make_material_request() {
		frappe.model.open_mapped_doc({
			method: "beasm.selling.doctype.sales_order.sales_order.make_material_request",
			frm: this.frm
		})
	}
	...
	make_raw_material_request_dialog(r) {
		...
		var d = new frappe.ui.Dialog({
			...
			primary_action: function() {
				...
				me.frm.call({
					method: 'beasm.selling.doctype.sales_order.sales_order.make_raw_material_request',
					args: {
						items: data,
						company: me.frm.doc.company,
						sales_order: me.frm.docname,
	...
	make_maintenance_schedule() {
		frappe.model.open_mapped_doc({
			method: "beasm.selling.doctype.sales_order.sales_order.make_maintenance_schedule",
			frm: this.frm
		})
	}
	...
	make_project() {
		frappe.model.open_mapped_doc({
			method: "beasm.selling.doctype.sales_order.sales_order.make_project",
			frm: this.frm
		})
	}
	...
	make_inter_company_order() {
		frappe.model.open_mapped_doc({
			method: "beasm.selling.doctype.sales_order.sales_order.make_inter_company_purchase_order",
			frm: this.frm
		});
	}
	...
	make_maintenance_visit() {
		frappe.model.open_mapped_doc({
			method: "beasm.selling.doctype.sales_order.sales_order.make_maintenance_visit",
			frm: this.frm
		})
	}
	...
	make_purchase_order(){
		...
		var dialog = new frappe.ui.Dialog({
			...
			primary_action (args) {
				...
				return frappe.call({
					method: "beasm.selling.doctype.sales_order.sales_order." + method,
	...
	update_status(label, status){
		...
		frappe.call({
			method: "beasm.selling.doctype.sales_order.sales_order.update_status",
...
extend_cscript(cur_frm.cscript, new beasm.selling.SalesOrderController({frm: cur_frm}));
