...
def check_setup_wizard_not_completed():
	if cint(frappe.db.get_single_value("System Settings", "setup_complete") or 0):
		message = """BEASM can only be installed on a fresh site where the setup wizard is not completed.
...
def check_frappe_version():
	...
	beasm_version = major_version(beasm.__version__)

	if frappe_version == beasm_version:
		return

	click.secho(
		f"You're attempting to install BEASM version {beasm_version} with Frappe version {frappe_version}. "
...
def add_app_name():
	frappe.db.set_single_value("System Settings", "app_name", "BEASM")
