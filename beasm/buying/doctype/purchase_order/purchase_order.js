...
beasm.accounts.taxes.setup_tax_filters("Purchase Taxes and Charges");
beasm.accounts.taxes.setup_tax_validations("Purchase Order");
beasm.buying.setup_buying_controller();
	...
	get_materials_from_supplier: function(frm) {
		...
		if (po_details && po_details.length) {
			frm.add_custom_button(__('Return of Components'), () => {
				frm.call({
					method: 'beasm.controllers.subcontracting_controller.get_materials_from_supplier',
					freeze: true,
	...
	schedule_date: function(frm, cdt, cdn) {
		...
		if (row.schedule_date) {
			if(!frm.doc.schedule_date) {
				beasm.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "items", "schedule_date");
			} else {
				set_schedule_date(frm);
			}
		}
...
beasm.buying.PurchaseOrderController = class PurchaseOrderController extends beasm.buying.BuyingController {
	setup() {
	...
	get_items_from_open_material_requests() {
		beasm.utils.map_current_doc({
			method: "beasm.stock.doctype.material_request.material_request.make_purchase_order_based_on_supplier",
			args: {
			...
			get_query_method: "beasm.stock.doctype.material_request.material_request.get_material_requests_based_on_supplier"
		});
	}
	...
	make_stock_entry() {
		frappe.call({
			method:"beasm.controllers.subcontracting_controller.make_rm_stock_entry",
			args: {
	...
	make_purchase_receipt() {
		frappe.model.open_mapped_doc({
			method: "beasm.buying.doctype.purchase_order.purchase_order.make_purchase_receipt",
			frm: cur_frm,
	...
	make_purchase_invoice() {
		frappe.model.open_mapped_doc({
			method: "beasm.buying.doctype.purchase_order.purchase_order.make_purchase_invoice",
			frm: cur_frm
		})
	}
	...
	make_subcontracting_order() {
		frappe.model.open_mapped_doc({
			method: "beasm.buying.doctype.purchase_order.purchase_order.make_subcontracting_order",
			frm: cur_frm,
	...
	add_from_mappers() {
		...
		this.frm.add_custom_button(__('Material Request'),
			function() {
				beasm.utils.map_current_doc({
					method: "beasm.stock.doctype.material_request.material_request.make_purchase_order",
					source_doctype: "Material Request",
					target: me.frm,
					setters: {
		...
		this.frm.add_custom_button(__('Supplier Quotation'),
			function() {
				beasm.utils.map_current_doc({
					method: "beasm.buying.doctype.supplier_quotation.supplier_quotation.make_purchase_order",
		...
		function() {
			...
			frappe.call({
				method: "beasm.buying.utils.get_linked_material_requests",
				args:{
					items: my_items
				},
				callback: function(r) {
...
extend_cscript(cur_frm.cscript, new beasm.buying.PurchaseOrderController({frm: cur_frm}));

cur_frm.cscript.update_status= function(label, status){
	frappe.call({
		method: "beasm.buying.doctype.purchase_order.purchase_order.update_status",
...
function set_schedule_date(frm) {
	if(frm.doc.schedule_date){
		beasm.utils.copy_value_in_all_rows(frm.doc, frm.doc.doctype, frm.doc.name, "items", "schedule_date");
	}
}
...
frappe.ui.form.on("Purchase Order", "is_subcontracted", function(frm) {
	if (frm.doc.is_old_subcontracting_flow) {
		beasm.buying.get_default_bom(frm);
	}
});